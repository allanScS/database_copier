...
public class MyHealthJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "my_health";
		final String targetTable = "myHealth";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "glucose",
				"health_condition_text", "pad", "pas", "updated_at", "updated_by", "weight", "patient_id" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, MyHealth.class);

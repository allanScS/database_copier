...
public class PatientCaseForwardingJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientCaseForwarding> converter = new AbstractConverter<PatientCaseForwarding>()
					.convertjsonToEntityList(PatientCaseForwarding.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientCaseForwarding> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientCaseForwarding entity = entityList.get(i);

...
public class PatientCaseEvolutionJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientCaseEvolution> converter = new AbstractConverter<PatientCaseEvolution>()
					.convertjsonToEntityList(PatientCaseEvolution.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientCaseEvolution> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientCaseEvolution entity = entityList.get(i);

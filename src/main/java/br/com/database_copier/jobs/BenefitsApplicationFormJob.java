...
public class BenefitsApplicationFormJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "benefits_application_form";
		final String targetTable = "benefitsApplicationForm";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "patient_case_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<BenefitsApplicationForm> converter = new AbstractConverter<BenefitsApplicationForm>()
					.convertjsonToEntityList(BenefitsApplicationForm.class, GenericUtils.objectsToJson(list, fields));

			List<BenefitsApplicationForm> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final BenefitsApplicationForm entity = entityList.get(i);

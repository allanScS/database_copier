...
public class SupplierAddressJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "supplier_address";
		final String targetTable = "supplierAddress";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<SupplierAddress> converter = new AbstractConverter<SupplierAddress>()
					.convertjsonToEntityList(SupplierAddress.class, GenericUtils.objectsToJson(list, fields));

			final List<SupplierAddress> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final SupplierAddress entity = entityList.get(i);

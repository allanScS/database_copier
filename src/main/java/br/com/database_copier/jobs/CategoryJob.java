...
public class CategoryJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "active", "category_type", "description", "color" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<Category> converter = new AbstractConverter<Category>()
					.convertjsonToEntityList(Category.class, GenericUtils.objectsToJson(list, fields));

			List<Category> entityList = converter.getEntities();
...
System.out.println("Transaction commited and Session closed");
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final Category entity = entityList.get(i);

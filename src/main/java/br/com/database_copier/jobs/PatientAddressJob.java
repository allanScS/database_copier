...
public class PatientAddressJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientAddress> converter = new AbstractConverter<PatientAddress>()
					.convertjsonToEntityList(PatientAddress.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientAddress> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientAddress entity = entityList.get(i);

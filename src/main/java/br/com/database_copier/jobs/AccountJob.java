...
public class AccountJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<Account> converter = new AbstractConverter<Account>()
					.convertjsonToEntityList(Account.class, GenericUtils.objectsToJson(list, fields));

			List<Account> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final Account entity = entityList.get(i);

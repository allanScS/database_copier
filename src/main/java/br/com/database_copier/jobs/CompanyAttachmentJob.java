...
public class CompanyAttachmentJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyAttachment> converter = new AbstractConverter<CompanyAttachment>()
					.convertjsonToEntityList(CompanyAttachment.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyAttachment> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyAttachment entity = entityList.get(i);

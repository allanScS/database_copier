...
public class WorkShiftJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "work_shift";
		final String targetTable = "workShift";

		final String[] fields = { "id", "active", "created_at", "created_by", "day_of_week", "deleted", "deleted_at",
				"deleted_by", "ends_at", "starts_at", "updated_at", "updated_by", "account_id" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, WorkShift.class);

...
public class ChannelJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "channel";
		final String targetTable = "channel";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "name",
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, Channel.class);

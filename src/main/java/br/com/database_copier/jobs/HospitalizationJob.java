...
public class HospitalizationJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "hospitalization";
		final String targetTable = "hospitalization";

		final String[] fields = { "id", "created_at", "created_by", "date", "deleted", "deleted_at", "deleted_by",
				"description", "hospital_name", "updated_at", "updated_by", "patient_id" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, Hospitalization.class);

...
public class CompanyHealthPlanJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "company_health_plan";
		final String targetTable = "companyHealthPlan";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "code", "coverage", "insurance", "insurance_policy_number", "plan", "company_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyHealthPlan> converter = new AbstractConverter<CompanyHealthPlan>()
					.convertjsonToEntityList(CompanyHealthPlan.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyHealthPlan> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyHealthPlan entity = entityList.get(i);

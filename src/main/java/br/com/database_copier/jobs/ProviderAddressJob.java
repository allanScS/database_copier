...
public class ProviderAddressJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "provider_address";
		final String targetTable = "providerAddress";
		...
		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				...
				"longitude", "number", "postal_code", "reference_point", "state", "street", "provider_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ProviderAddress> converter = new AbstractConverter<ProviderAddress>()
					.convertjsonToEntityList(ProviderAddress.class, GenericUtils.objectsToJson(list, fields));

			final List<ProviderAddress> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ProviderAddress entity = entityList.get(i);

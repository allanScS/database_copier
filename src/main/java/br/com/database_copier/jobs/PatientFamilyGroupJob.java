...
public class PatientFamilyGroupJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "patient_family_group";
		final String targetTable = "patientFamilyGroup";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientFamilyGroup> converter = new AbstractConverter<PatientFamilyGroup>()
					.convertjsonToEntityList(PatientFamilyGroup.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientFamilyGroup> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientFamilyGroup entity = entityList.get(i);
				...
				if (entity.getRelativeId() != null) {
					entity.setRelative(new Patient());
					entity.getRelative().setId(entity.getRelativeId());
				}

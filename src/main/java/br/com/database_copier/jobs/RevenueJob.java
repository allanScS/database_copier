...
public class RevenueJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				...
				"installments_number", "installment_value", "client_id", "category_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<Revenue> converter = new AbstractConverter<Revenue>()
					.convertjsonToEntityList(Revenue.class, GenericUtils.objectsToJson(list, fields));

			final List<Revenue> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final Revenue entity = entityList.get(i);

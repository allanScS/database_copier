...
public class CompanyEmailJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "company_email";
		final String targetTable = "companyEmail";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "description", "favourite", "company_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyEmail> converter = new AbstractConverter<CompanyEmail>()
					.convertjsonToEntityList(CompanyEmail.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyEmail> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyEmail entity = entityList.get(i);

...
public class CompanyPhoneJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "company_phone";
		final String targetTable = "companyPhone";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyPhone> converter = new AbstractConverter<CompanyPhone>()
					.convertjsonToEntityList(CompanyPhone.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyPhone> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyPhone entity = entityList.get(i);

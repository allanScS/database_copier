...
public class PatientPhoneJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "patient_phone";
		final String targetTable = "patientPhone";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "description", "favourite", "patient_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientPhone> converter = new AbstractConverter<PatientPhone>()
					.convertjsonToEntityList(PatientPhone.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientPhone> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientPhone entity = entityList.get(i);

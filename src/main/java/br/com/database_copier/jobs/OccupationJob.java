...
public class OccupationJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<Occupation> converter = new AbstractConverter<Occupation>()
					.convertjsonToEntityList(Occupation.class, GenericUtils.objectsToJson(list, fields));

			final List<Occupation> entityList = converter.getEntities();
...
System.out.println("Transaction commited and Session closed");
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final Occupation entity = entityList.get(i);

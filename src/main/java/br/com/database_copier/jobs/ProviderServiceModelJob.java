...
public class ProviderServiceModelJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "provider_service_model";
		final String targetTable = "providerServiceModel";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ProviderServiceModel> converter = new AbstractConverter<ProviderServiceModel>()
					.convertjsonToEntityList(ProviderServiceModel.class, GenericUtils.objectsToJson(list, fields));

			final List<ProviderServiceModel> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ProviderServiceModel entity = entityList.get(i);

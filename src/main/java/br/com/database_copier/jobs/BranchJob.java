...
public class BranchJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<Branch> converter = new AbstractConverter<Branch>()
					.convertjsonToEntityList(Branch.class, GenericUtils.objectsToJson(list, fields));

			final List<Branch> entityList = converter.getEntities();
...
System.out.println("Transaction commited and Session closed");
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final Branch entity = entityList.get(i);

...
public class PatientAttachmentJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "patient_attachment";
		final String targetTable = "patientAttachment";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "file_extension", "filename", "patient_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientAttachment> converter = new AbstractConverter<PatientAttachment>()
					.convertjsonToEntityList(PatientAttachment.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientAttachment> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientAttachment entity = entityList.get(i);

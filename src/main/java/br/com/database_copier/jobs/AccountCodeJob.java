...
public class AccountCodeJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<AccountCode> converter = new AbstractConverter<AccountCode>()
					.convertjsonToEntityList(AccountCode.class, GenericUtils.objectsToJson(list, fields));

			List<AccountCode> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final AccountCode entity = entityList.get(i);

...
public class LevelJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<Level> converter = new AbstractConverter<Level>()
					.convertjsonToEntityList(Level.class, GenericUtils.objectsToJson(list, fields));

			final List<Level> entityList = converter.getEntities();
...
System.out.println("Transaction commited and Session closed");
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final Level entity = entityList.get(i);

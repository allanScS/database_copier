...
public class ProviderPaymentJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ProviderPayment> converter = new AbstractConverter<ProviderPayment>()
					.convertjsonToEntityList(ProviderPayment.class, GenericUtils.objectsToJson(list, fields));

			final List<ProviderPayment> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ProviderPayment entity = entityList.get(i);

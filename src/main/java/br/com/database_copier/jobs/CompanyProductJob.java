...
public class CompanyProductJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "company_product";
		final String targetTable = "companyProduct";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "product", "company_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyProduct> converter = new AbstractConverter<CompanyProduct>()
					.convertjsonToEntityList(CompanyProduct.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyProduct> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyProduct entity = entityList.get(i);

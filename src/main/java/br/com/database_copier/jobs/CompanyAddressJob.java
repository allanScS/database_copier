...
public class CompanyAddressJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyAddress> converter = new AbstractConverter<CompanyAddress>()
					.convertjsonToEntityList(CompanyAddress.class, GenericUtils.objectsToJson(list, fields));

			List<CompanyAddress> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyAddress entity = entityList.get(i);

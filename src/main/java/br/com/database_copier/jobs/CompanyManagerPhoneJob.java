...
public class CompanyManagerPhoneJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "company_manager_phone";
		final String targetTable = "companyManagerPhone";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "description", "favourite", "company_manager_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyManagerPhone> converter = new AbstractConverter<CompanyManagerPhone>()
					.convertjsonToEntityList(CompanyManagerPhone.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyManagerPhone> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyManagerPhone entity = entityList.get(i);

				if (entity.getCompanyManagerId() != null) {
					entity.setCompanyManager(new CompanyManager());
					entity.getCompanyManager().setId(entity.getCompanyManagerId());
				}

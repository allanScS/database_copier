...
public class SupplierPhoneJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "supplier_phone";
		final String targetTable = "supplierPhone";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "description", "favourite", "supplier_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<SupplierPhone> converter = new AbstractConverter<SupplierPhone>()
					.convertjsonToEntityList(SupplierPhone.class, GenericUtils.objectsToJson(list, fields));

			final List<SupplierPhone> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final SupplierPhone entity = entityList.get(i);

...
public class CompanyManagerJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "company_manager";
		final String targetTable = "companyManager";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "department", "name", "role", "company_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CompanyManager> converter = new AbstractConverter<CompanyManager>()
					.convertjsonToEntityList(CompanyManager.class, GenericUtils.objectsToJson(list, fields));

			final List<CompanyManager> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CompanyManager entity = entityList.get(i);

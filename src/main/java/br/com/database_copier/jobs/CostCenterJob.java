...
public class CostCenterJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "cost_center";
		final String targetTable = "costCenter";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "code", "name", "company_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<CostCenter> converter = new AbstractConverter<CostCenter>()
					.convertjsonToEntityList(CostCenter.class, GenericUtils.objectsToJson(list, fields));

			final List<CostCenter> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final CostCenter entity = entityList.get(i);

...
public class TroubleAreaJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "trouble_area";
		final String targetTable = "troubleArea";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<TroubleArea> converter = new AbstractConverter<TroubleArea>()
					.convertjsonToEntityList(TroubleArea.class, GenericUtils.objectsToJson(list, fields));

			final List<TroubleArea> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final TroubleArea entity = entityList.get(i);

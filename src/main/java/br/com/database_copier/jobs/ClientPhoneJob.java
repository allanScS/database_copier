...
public class ClientPhoneJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "client_phone";
		final String targetTable = "clientPhone";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "description", "favourite", "client_id" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ClientPhone> converter = new AbstractConverter<ClientPhone>()
					.convertjsonToEntityList(ClientPhone.class, GenericUtils.objectsToJson(list, fields));

			List<ClientPhone> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ClientPhone entity = entityList.get(i);

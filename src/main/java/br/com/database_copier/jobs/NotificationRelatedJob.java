...
public class NotificationRelatedJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "notification_related";
		final String targetTable = "notificationRelated";

		final String[] fields = { "notification_id", "related_id" };

		final BigInteger totalElements = (BigInteger) source
				.createNativeQuery("SELECT COUNT(entity.notification_id) FROM " + GenericUtils.SOURCE_SCHEMA + "."
						+ sourceTable + " AS entity")
				.uniqueResult();

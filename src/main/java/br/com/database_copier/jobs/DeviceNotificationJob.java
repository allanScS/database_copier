...
public class DeviceNotificationJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "device_notification";
		final String targetTable = "deviceNotification";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "reference",
				"sended_at", "sound", "status", "title", "type", "updated_at", "updated_by", "viewed_at", "account_id",
				"body", "category_id", "category_identifier", "expiration", "reference_name", "channel_id",
				"content_available" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, DeviceNotification.class);

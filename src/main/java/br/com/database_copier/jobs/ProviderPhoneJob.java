...
public class ProviderPhoneJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ProviderPhone> converter = new AbstractConverter<ProviderPhone>()
					.convertjsonToEntityList(ProviderPhone.class, GenericUtils.objectsToJson(list, fields));

			final List<ProviderPhone> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ProviderPhone entity = entityList.get(i);

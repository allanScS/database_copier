...
public class QuestionsAndAnswersJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "questions_and_answers";
		final String targetTable = "questionsAndAnswers";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<QuestionsAndAnswers> converter = new AbstractConverter<QuestionsAndAnswers>()
					.convertjsonToEntityList(QuestionsAndAnswers.class, GenericUtils.objectsToJson(list, fields));

			final List<QuestionsAndAnswers> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final QuestionsAndAnswers entity = entityList.get(i);

				if (entity.getCustomerSurveyId() != null) {
					entity.setCustomerSurvey(new CustomerSurvey());
					entity.getCustomerSurvey().setId(entity.getCustomerSurveyId());
				}

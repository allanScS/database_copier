...
public class ProviderHealthPlanJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "provider_health_plan";
		final String targetTable = "providerHealthPlan";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ProviderHealthPlan> converter = new AbstractConverter<ProviderHealthPlan>()
					.convertjsonToEntityList(ProviderHealthPlan.class, GenericUtils.objectsToJson(list, fields));

			final List<ProviderHealthPlan> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ProviderHealthPlan entity = entityList.get(i);

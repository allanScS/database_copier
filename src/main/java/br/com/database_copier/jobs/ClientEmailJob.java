...
public class ClientEmailJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "client_email";
		final String targetTable = "clientEmail";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ClientEmail> converter = new AbstractConverter<ClientEmail>()
					.convertjsonToEntityList(ClientEmail.class, GenericUtils.objectsToJson(list, fields));

			List<ClientEmail> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ClientEmail entity = entityList.get(i);

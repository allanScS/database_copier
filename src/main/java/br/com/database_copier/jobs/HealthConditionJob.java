...
public class HealthConditionJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "health_condition";
		final String targetTable = "healthCondition";

		final String[] fields = { "id", "deleted", "deleted_at", "deleted_by", "gender_affected", "name", "tag_code" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, HealthCondition.class);

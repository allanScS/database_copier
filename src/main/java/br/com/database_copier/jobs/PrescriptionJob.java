...
public class PrescriptionJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "prescription";
		final String targetTable = "prescription";

		final String[] fields = { "id", "created_at", "created_by", "image_url", "name", "updated_at", "updated_by",
				"account_id", "share_to_backoffice" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, Prescription.class);

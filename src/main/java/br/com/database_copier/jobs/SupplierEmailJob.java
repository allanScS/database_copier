...
public class SupplierEmailJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "supplier_email";
		final String targetTable = "supplierEmail";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<SupplierEmail> converter = new AbstractConverter<SupplierEmail>()
					.convertjsonToEntityList(SupplierEmail.class, GenericUtils.objectsToJson(list, fields));

			final List<SupplierEmail> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final SupplierEmail entity = entityList.get(i);

...
public class PatientImportNaturaDataJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "patient_import_natura_data";
		final String targetTable = "patientImportNaturaData";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "updated_at",
				"updated_by", "bunch", "cd_consultant", "home_time", "name", "profile", "sales_management", "sector",
				"state", "strategic_region", "tax_number" };
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<PatientImportNaturaData> converter = new AbstractConverter<PatientImportNaturaData>()
					.convertjsonToEntityList(PatientImportNaturaData.class, GenericUtils.objectsToJson(list, fields));

			final List<PatientImportNaturaData> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final PatientImportNaturaData entity = entityList.get(i);

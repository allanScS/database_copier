...
public class InvitationJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "invitation";
		final String targetTable = "invitation";

		final String[] fields = { "id", "accepted", "active", "created_at", "created_by", "reacted_at", "updated_at",
				"updated_by", "call_id", "guest_id", "invite_sender_id" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, Invitation.class);

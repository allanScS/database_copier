...
public class ProviderAttachmentJob {
	...
	public static void execute(Integer itensPerPage) {
		...
		final String sourceTable = "client_attachment";
		final String targetTable = "ProviderAttachment";
		...
		while (hasMoreElements) {
			...
			final AbstractConverter<ProviderAttachment> converter = new AbstractConverter<ProviderAttachment>()
					.convertjsonToEntityList(ProviderAttachment.class, GenericUtils.objectsToJson(list, fields));

			final List<ProviderAttachment> entityList = converter.getEntities();
			...
			for (i = 0; i < entityList.size(); i++) {
				...
				final ProviderAttachment entity = entityList.get(i);

...
public class RefreshTokenJob {
	...
	public static void execute(final Integer itensPerPage, final Integer poolLimit, final Session source) {
		...
		final String sourceTable = "refresh_token";
		final String targetTable = "refreshToken";

		final String[] fields = { "id", "created_at", "created_by", "deleted", "deleted_at", "deleted_by", "expired",
				"refresh_token", "updated_at", "updated_by", "account_id" };
			...
			threadPool.execute(() -> {
				try {
					...
					executePageUtil.executePage(fields.clone(), sourceTable, targetTable, itensPerPage, page2,
							totalPages, source, RefreshToken.class);
